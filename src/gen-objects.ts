...
export function addImageDefinition (target: Container, slide: PresSlide, opt: ImageProps): void {
	...
	const objectName = opt.objectName ? encodeXmlEntities(opt.objectName) : `Image ${slide._slideObjects.filter(obj => obj._type === SLIDE_OBJECT_TYPES.image).length}`
	...
	function cleanOpts (itemOpts: ObjectOptions): TextPropsOptions {
		// STEP 1: Set some options
		{
			...
			itemOpts.objectName = itemOpts.objectName
				...
				: `Text ${slide._slideObjects.filter(obj => obj._type === SLIDE_OBJECT_TYPES.text).length}`
